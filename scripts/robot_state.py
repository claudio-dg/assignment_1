...
class RobotState:
    ...
    def __init__(self):
        ...
        if self._randomness:
            self._random_battery_time = rospy.get_param(anm.PARAM_BATTERY_TIME, [250.0, 450.0, 20.0,35.0])# [25.0, 45.0, 20.0,35.0] The delay between changes of battery levels, i.e., high/low.
        ...
        # Define services.
        rospy.Service(anm.SERVER_GET_POSE, GetPose, self.get_pose_callback) ######### e' l info/ask CANCELLARE ########

        rospy.Service(anm.SERVER_SET_POSE, SetPose, self.set_pose_callback)
        # FORSE QUESTO NON SERVE PIU PERCHE LA setto con la sub la pose! pero lascio per ora perche puo servire a inizalizzare in  helper  (Def_init_pose he viene chiamata in fsm)
        # credo che potrei fare stessa cosa chiamando invece il getPose mettendo un waitFor service per inizializzare..ma per non cambiare troppo la
        # struttura provo a lasciare cosi'
    ...
    def get_pose_callback(self, request): #la get rimane uguale...
        # Log information.
        if self._pose is None:
            rospy.logerr(anm.tag_log('Cannot get an unspecified robot position', LOG_TAG))
        # Create the response with the robot pose and return it.
