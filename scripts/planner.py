...
class PlaningAction(object):
    ...
    def execute_callback(self, goal):
        ...
        if start_point is None or target_point is None:
            ...
            return
        # if not(self._is_valid(start_point) and self._is_valid(target_point)): NON POSSO SAPERE SE E VALID
        ...
        # Publish the feedback and ######## CANCLLA STA PARTE DEL WAIT wait to simulate computation.
        self._as.publish_feedback(feedback)
        # delay = random.uniform(self._random_plan_time[0], self._random_plan_time[1])
        # rospy.sleep(delay) # ORA NON SERVE PIU QUESTA WAIT

        # Get a random number of via points to be included in the plan. ########COMMENTO TUTTO QUESTO! MANDO SOLO START E GOAL!!
    ...
    # Check if the point is within the environment bounds, i.e. ####cancello non posso sapere se e valis
